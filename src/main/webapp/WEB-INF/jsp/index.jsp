...
app.controller("TaskController", function($scope, $http) {
	...
	// HTTP POST/PUT methods for add/edit employee
	    // Call: http://localhost:8080/employee
	        ...
	        $http({
	            method: "POST",
	            url: 'http://localhost:8060/addUser',
	    ...
	    function getUsers() {	    
           $http({
               method : 'GET',
               url : 'http://localhost:8060/getUsers',
                headers: {
                   'Content-Type': 'application/json'
               } 
           }).then(function successCallback(data) {
       ...
       function getTask() {
         $http({
               method : 'GET',
               url : 'http://localhost:8060/getTasks',
                headers: {
                   'Content-Type': 'application/json'
               } 
           }).then(function successCallback(data) {
       ...
       function getActiveTask() {
           $http({
                 method : 'GET',
                 url : 'http://localhost:8060/getActiveTasks',
                  headers: {
                     'Content-Type': 'application/json'
                 } 
             }).then(function successCallback(data) {
       ...
       function getProjects() {	    
           $http({
               method : 'GET',
               url : 'http://localhost:8060/getProjects',
                headers: {
                   'Content-Type': 'application/json'
               } 
           }).then(function successCallback(data) {
       ...
       $scope.updateUserData = function (evt, user) {
	       ...
	       // alert($scope.userForm);
	        ...
	        $http({
	            method: "POST",
	            url: 'http://localhost:8060/updateUser',
	            data: angular.toJson($scope.userForm),
	            headers: {
	                'Content-Type': 'application/json'
	            }
	...
	$scope.deleteUser = function (evt, userId) {
       // alert("Entered into End "+taskId);
           $http({
               method : 'GET',
               url : 'http://localhost:8060/deleteUser/'+userId,
    ...
    var emptyString  ="";
	   ...
	   $scope.searchUser = function () {
         if($scope.searchfname.trim() != "") {
           $http({
               method : 'GET',
               url : 'http://localhost:8060/searchUserByFirstName/'+$scope.searchfname,
               data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
             ...
             $('#firstname').val($scope.userForm.firstname);
               ...
               alert("User Not Found, Enter valid First Name!");
            	...
            	$http({
              	 method : 'GET',
              	 url : 'http://localhost:8060/searchUserByLastName/'+$scope.searchlname,
               	data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
            ...
            var tasksObj = data;
             ...
             $('#firstname').val($scope.userForm.firstname);
               ...
               console.log(response.statusText);
           		...
           		$http({
              	 method : 'GET',
              	 url : 'http://localhost:8060/searchUserByEmpID/'+$scope.searchempId,
               	data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
   ...
   // });
    ...
    function createProjectTask(projectName,task){
    	...
    	$http({
            method : 'GET',
            url : 'http://localhost:8060/fetchProjectByProjectId/'+pro,
    ...
    $scope.managerSearch = function(){
    	$http({
               method : 'GET',
               url : 'http://localhost:8060/searchUserByFirstName/'+$scope.searchManager,
               data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
...
// Now add the data to server
 ...
 $scope.addTaskData = function (evt, taskName) {
	// HTTP POST/PUT methods for add/edit employee
	    // Call: http://localhost:8080/employee
	        ...
	        $http({
	            method: "POST",
	            url: 'http://localhost:8060/addTask',
 ...
 $scope.update = function (evt, taskName) {
	...
	$scope.taskForm.taskName = $('#ntask').val();
	        ...
	        $http({
	            method: "POST",
	            url: 'http://localhost:8060/updateTask',
	            data: angular.toJson($scope.taskForm),
	            headers: {
	                'Content-Type': 'application/json'
	            }
   ...
   $scope.endTaskData = function (evt, task) {
        ...
        $scope.taskForm.status =  "DeActive";
           ...
           $http({
	           ...
	           data: angular.toJson($scope.taskForm),
              // url :
				// 'http://localhost:8060/deleteTask/'+taskId,
              // data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
  ...
  $scope.deleteTask = function (event, taskId){
 	 $http({
               method : 'GET',
               url : 'http://localhost:8060/deleteTask/'+taskId,
               data: angular.toJson($scope.task),
                headers: {
                   'Content-Type': 'application/json'
               } 
            ...
            <tbody ng-repeat="task in activeTasks">
                ...
                <tr>
                	...
                	<!--<td><a href="http://localhost:8060/editTask/{{task.id}}" ng-click="editTask(event, {{task}})"/>Edit</a></td>-->
   					<!--<td><a href="http://localhost:8060/deleteTask/{{task.id}}"  ng-click="deleteTask()"/>Delete</a></td>-->
		   ...
		   <table cellspacing="100" cellpadding="10" class="calc">		
            ...
            <tbody ng-repeat="task in tasks">
                ...
                <tr>
					...
					<!--->
                	<!--<td><a href="http://localhost:8060/editTask/{{task.id}}" ng-click="editTask(event, {{task}})"/>Edit</a></td>-->
   					<!--<td><a href="http://localhost:8060/deleteTask/{{task.id}}"  ng-click="endTask()"/>Delete</a></td>-->
